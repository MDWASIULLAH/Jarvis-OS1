...
from ..capabilities.code_executor import run_python
...
#: Which kind of engine each intent actually needs. The router turns this into
#: a concrete provider so "auto" (the API default) picks per request instead of
#: pinning every turn to whatever the UI dropdown was last left on: arithmetic
#: and the clock stay local, writing code or prose goes to a generative model.
_TASK_BY_INTENT: dict[str, str] = {
    "info.math": "math",
    "info.time": "clock",
    ...
    "info.definition": "reasoning",
    "info.factual": "reasoning",
    "media.image_search": "media",
    "media.video_search": "media",
    "media.image_generate": "media",
    "web.browse": "reasoning",
    "action.open_app": "action",
    "action.web_open": "action",
    "action.screenshot": "action",
    "action.system_control": "system",
...
def task_for(intent: str) -> str:
    if intent.startswith("smalltalk"):
        return "smalltalk"
    return _TASK_BY_INTENT.get(intent, "reasoning")
...
_GENERIC_CODE_REQUEST_RE = re.compile(
    r"^\s*(?:please\s+)?(?:write|create|make|generate|build)\s+(?:a\s+)?code(?:\s+for\s+me)?\s*$",
    re.IGNORECASE,
)

# ---- running code the user pasted
#
# `code_executor.run_python` was written, tested, and then never called: the
# only handler for `task.code` generated text, so pasting a snippet and asking
# for its output produced a "tell me your requirements" template instead of the
# output. These patterns pull the runnable snippet back out of the message.
#
# A fenced block is unambiguous. A `run this:` lead-in is next best. Falling
# back to "treat the whole message as code" needs care -- `compile()` happily
# accepts a bare word like "hello" as an expression statement -- so that path
# additionally demands a newline and a real Python token.
_FENCED_CODE_RE = re.compile(r"```[ \t]*(\w+)?[ \t]*\r?\n(.*?)```", re.S)
_RUN_LEAD_IN_RE = re.compile(
    r"^\s*(?:please\s+)?(?:run|execute|eval(?:uate)?|debug|test|fix|check)\b[^\n:]*?:\s*",
    re.IGNORECASE,
)
_PYTHON_TOKEN_RE = re.compile(
    r"(?:^|\n)\s*(?:def|class|import|from|for|while|if|with|try|print|return|assert)\b|=|\bprint\s*\(",
)
_RUNNABLE_LANGS = {"", "py", "python", "python3"}
_TEMP_SCRIPT_RE = re.compile(r'File "[^"]*[/\\]tmp\w+\.py"')


# A grounded answer is asked to admit when the retrieved evidence falls short.
# Recognising that admission is what lets us retry unsourced instead of handing
# the user a refusal. Length is half the signal: a substantive answer that
# happens to contain "does not mention" is long, a bare refusal is a sentence or
# two, so the caller only trusts this on short replies.
_EVIDENCE_GAP_RE = re.compile(
    r"\b(?:evidence|sources?|context|search results?|information provided)\b[^.]{0,80}?"
    r"\b(?:does not|doesn't|do not|don't|lacks?|fails? to|cannot|can't)\b"
    r"|\b(?:does not|doesn't|do not|don't)\s+(?:contain|include|mention|provide|address|explain|specify)\b"
    r"|\bno (?:information|mention|explanation|details?|answer)\b",
    re.I,
)

_GAP_REPLY_MAX_CHARS = 420


def _looks_like_evidence_gap(reply: str) -> bool:
    """True when `reply` is a short "the evidence doesn't cover this" admission."""
    return len(reply) <= _GAP_REPLY_MAX_CHARS and bool(_EVIDENCE_GAP_RE.search(reply))


def _compiles(code: str) -> bool:
    """True when `code` is syntactically valid Python. Compiling never runs it."""
    try:
...
def _clean_traceback(stderr: str) -> str:
    """Replace the throwaway temp-file path with a label the user recognises.

    run_python writes the snippet to a tempfile, so a traceback otherwise reads
    `File "C:\\...\\Temp\\tmpx9i_3vdr.py", line 2`, which looks like a fault in
    some file the user has never seen.
    """
    return _TEMP_SCRIPT_RE.sub('File "your snippet"', stderr.rstrip())


def extract_python(text: str) -> Optional[str]:
    """The runnable Python in `text`, or None if it is just a prose request.

    Deliberately conservative: a false positive here would execute something
    the user only meant as a description.
    """
    for language, body in _FENCED_CODE_RE.findall(text):
        if (language or "").lower() in _RUNNABLE_LANGS and body.strip():
            return body.strip("\n")

    lead_in = _RUN_LEAD_IN_RE.match(text)
    if lead_in:
        remainder = text[lead_in.end():].strip()
        # Not requiring _compiles here on purpose: "debug this: <broken code>"
        # is the whole point of the debug path, and a syntax error is the answer
        # the user wants. The explicit verb plus a Python token is signal enough.
        if remainder and _PYTHON_TOKEN_RE.search(remainder):
            return remainder

    stripped = text.strip()
    if "\n" in stripped and _PYTHON_TOKEN_RE.search(stripped) and _compiles(stripped):
        return stripped
    return None


def _format_execution(result) -> str:
    """Render a real ExecutionResult -- including failures, which are the point
    of "debug this" -- instead of claiming success."""
    sections: list[str] = []
    if result.stdout.strip():
        sections.append(f"**Output**\n```\n{result.stdout.rstrip()}\n```")
    if result.stderr.strip():
        label = "Timed out" if result.timed_out else "Errors"
        sections.append(f"**{label}**\n```\n{_clean_traceback(result.stderr)}\n```")
    if not sections:
        sections.append("The code ran and produced no output.")
    if result.timed_out:
        sections.append("The snippet was killed for running too long, so the output above may be partial.")
    elif result.exit_code != 0:
        sections.append(f"It exited with code {result.exit_code}. The traceback above shows where it failed.")
    return "\n\n".join(sections)
...
class BrainCore:
    ...
    def __init__(self, runtime: Any):
        ...
        self.decision_engine = DecisionEngine(event_bus=runtime.events)
        self.intent_router = IntentRouter(self.intents, event_bus=runtime.events)
        self._reflect_history: list[dict] = []

    # ---- public entry point

    def think(self, text: str, provider: str = "auto", attachment_text: str = "") -> Thought:
        prediction = self.intents.analyze(text)
        correlation_id = str(uuid.uuid4())
        routing = self.intent_router.analyze(text, prediction, correlation_id=correlation_id)
        thought = Thought(
            text=text,
            intent=routing.intent,
            confidence=routing.confidence,
            intent_source=str(routing.metadata.get("source", prediction.source)),
            slots={**routing.entities, **dict(routing.metadata.get("slots", {}))},
        )
        if attachment_text:
            thought.intent = "vision.analyze" if thought.intent.startswith("smalltalk") else thought.intent
            if thought.intent != routing.intent:
                routing = routing.with_intent(thought.intent)
        thought.plan = plan_for(thought.intent)

        # Pick the engine from the request itself. A pasted snippet or a fenced
        # block is a coding turn no matter what the classifier said, so the
        # route is decided on the same evidence `_act` dispatches on.
        task = task_for(thought.intent)
        if task not in {"code", "vision"} and extract_python(text) is not None:
            task = "code"
        thought.route = self.rt.models.resolve_route(task, provider)
        thought.route["requested"] = provider
        resolved = str(thought.route.get("provider", "local"))

        llm_available = bool(self.rt.models.status().get("local_available"))
        decision = self.decision_engine.decide(
            routing,
            registry=self.rt.capabilities,
        ...
        try:
            if _combined_image_pattern.search(text):
                self._act(thought, prediction, resolved, attachment_text)
        ...
        # The route above is a decision; this is what happened. A deterministic
        # turn (a sum, the clock, an image render) answers without a model at
        # all, and the UI should say so rather than name an engine that never
        # ran.
        model_calls = [c for c in thought.tools if c.ok and c.name in {"language_model", "code_generation"}]
        thought.route["used"] = bool(model_calls)
        thought.route["answered_by"] = (
            model_calls[-1].detail if model_calls else next((c.name for c in thought.tools if c.ok), "local_reasoning")
        )
    ...
    def _act(self, t: Thought, prediction: IntentPrediction, provider: str, attachment_text: str) -> None:
        handler = {
            ...
            # Wrapped so an explicit "cloud" request reaches code generation too;
            # every other handler is provider-independent.
            "task.code": lambda thought: self._h_code_task(thought, provider),
        }.get(t.intent)

        if attachment_text and t.intent in {"vision.analyze", "doc.read"}:
            return self._h_attachment(t, attachment_text)
        # A pasted snippet is executable evidence, so it outranks the classifier
        # entirely. Gating this on intent did not work: "run this: print(...)"
        # lands on info.math (where the calculator answered "I couldn't evaluate"
        # for code that runs fine), and a fenced block usually lands on `unknown`,
        # which fell through to a web search. `extract_python` is the conservative
        # part -- it only fires on a fenced Python block or an explicit
        # run/debug lead-in carrying real Python tokens.
        snippet = extract_python(t.text)
        if snippet is not None and self._run_snippet(t, snippet):
            return
        if handler:
            return handler(t)
    ...
    def _reroute(self, t: Thought, task: str) -> str:
        """Re-pick the engine when a handler falls through to another task.

        A deterministic handler routes to the local engine because it does not
        need a model at all. When it finds nothing ("what is a monad?" reaching
        the dictionary, an unparseable expression reaching the calculator) the
        turn becomes a reasoning turn, and it has to be routed as one --
        otherwise the fall-through silently answers with template composition
        on a machine that has a generative model configured.
        """
        requested = str(t.route.get("requested", "auto"))
        route = self.rt.models.resolve_route(task, requested)
        route["requested"] = requested
        route["rerouted_from"] = t.route.get("task", "")
        t.route = route
    ...
    def _h_math(self, t: Thought) -> None:
        ...
        if not result:
            return self._h_knowledge(t, self._reroute(t, "reasoning"), "")
        if "error" in result:
    ...
    def _h_definition(self, t: Thought) -> None:
        ...
        self._h_knowledge(t, self._reroute(t, "reasoning"), "")
    ...
    def _h_translate(self, t: Thought) -> None:
        ...
        if ok:
            t.tools.append(ToolCall("translate", True, f"{language}"))
            t.reply = f'"{phrase}" in {language.title()}: **{output}**'
            return
        t.tools.append(ToolCall("translate", False, "libretranslate unavailable"))
        # LibreTranslate is optional; a routed generative model translates just
        # as well, so a missing local service should degrade rather than refuse.
        engine = self.rt.models.generative_provider(str(t.route.get("provider", "auto")))
        if engine:
            rendered = self.rt.models.generate(
                f'Translate this into {language.title()} and reply with the translation only:\n{phrase}',
                system="You are a precise translator. Output only the translation, no commentary.",
                preference=engine,
            ).strip()
            if rendered:
                t.tools.append(ToolCall("language_model", True, f"{engine}, translation"))
                t.reply = f'"{phrase}" in {language.title()}: **{rendered}**'
                return
        t.reply = (
            "My translation backend (LibreTranslate) isn't running and no generative model is "
            "configured, so I won't guess a translation. Start one and set JARVIS_TRANSLATE_URL, "
            "or configure a cloud model, and this works immediately."
        )
    ...
    def _h_knowledge(self, t: Thought, provider: str, attachment_text: str) -> None:
        ...
        # Resolve by capability, not by the literal string the caller sent: a
        # configured cloud key used to be ignored here unless the user manually
        # flipped the provider selector, so evidence-grounded answers fell back
        # to template composition on a machine that could have written prose.
        engine = self.rt.models.generative_provider(provider)
        if engine:
            ...
            # Web results arrive as titles plus one-line descriptions, which is
            # thin evidence for a conceptual question. Grounding strictly on it
            # made the model answer "the evidence does not contain an explanation
            # of what a Python decorator is" -- a refusal for something any model
            # can explain. Retry once unsourced rather than refuse, and label it
            # so a grounded answer is never confused with an unsourced one.
            if _looks_like_evidence_gap(reply):
                unsourced = self.rt.models.generate(
                    question,
                    system=(
                        "You are JARVIS. Answer the question directly and accurately from your own "
                        "knowledge. Be concrete and concise. If you are genuinely unsure, say so."
                    ),
                    preference=engine,
                ).strip()
                if unsourced and not _looks_like_evidence_gap(unsourced):
                    t.tools.append(ToolCall("language_model", True, f"{engine}, unsourced"))
                    t.reply = (
                        f"{unsourced}\n\n"
                        "_Answered from the model's own knowledge — the sources I found didn't cover this._"
                    )
                    return

            t.tools.append(ToolCall("language_model", True, engine))
            t.reply = reply or self.local.compose(question, evidence, t.intent)
            # The prompt asks for [1]/[2] markers, so without the source list
            # the reply cited numbers that appeared nowhere on screen.
    ...
    def _h_code_task(self, t: Thought, provider: str = "local") -> None:
        question = t.text.strip()
        system = (
            "You are JARVIS, a coding assistant. Produce safe, testable, well-documented code. "
            "Include imports, error handling, and usage examples. "
            "Never claim commands ran unless they did. Be concise."
        )
        # Pasted snippets are already intercepted in `_act`, which sees both
        # intents the executor supports. Anything reaching here wants new code.
        if _GENERIC_CODE_REQUEST_RE.match(question):
            t.tools.append(ToolCall("code_generation", True, "clarification_template"))
            t.reply = self.local.generate(question, system=system)
            return
        # The old gate accepted `local_available`, which is true whenever the
        # deterministic engine is healthy -- i.e. always. So this branch always
        # ran, always with preference="local", and reported detail="LLM" while
        # actually returning the local engine's clarification template. A
        # configured cloud key could never win. Resolve by capability instead.
        engine = self.rt.models.generative_provider(provider)
        if engine:
            reply = self.rt.models.generate(
                f"Write code for this request:\n{question}",
                system=system,
                preference=engine,
            )
            stripped = reply.strip()
            if stripped and "don't have a reliable source" not in stripped.lower() and "won't guess" not in stripped.lower():
                t.tools.append(ToolCall("code_generation", True, engine))
                t.reply = stripped
                return
            # Fall through rather than tag a refusal as a successful generation.
        t.tools.append(ToolCall("code_generation", True, "local_reasoning"))
        t.reply = self.local.generate(question, system=system)

    def _run_snippet(self, t: Thought, snippet: str) -> bool:
        """Execute a pasted snippet. Returns False to fall through to generation.

        The `code_runner` toggle is honoured here. Before this, the toggle was
        only ever read back by `GET /v1/tools`, so switching it off in the UI
        changed nothing about what actually ran.
        """
        if not self.rt.get_tool_enabled("code_runner"):
            t.tools.append(ToolCall("code_execution", False, "disabled"))
            t.reply = (
                "The Code Runner tool is switched off, so I won't execute that snippet.\n\n"
                "Enable it under Settings → Capabilities & tools and send it again."
            )
            return True
        if not _compiles(snippet):
            # A snippet that will not compile is exactly the "debug this" case:
            # report the syntax error rather than pretending to run it.
            try:
                compile(snippet, "<snippet>", "exec")
            except (SyntaxError, ValueError) as exc:
                line = getattr(exc, "lineno", None)
                where = f" on line {line}" if line else ""
                t.tools.append(ToolCall("code_execution", True, "syntax_error"))
                t.reply = (
                    f"That snippet has a syntax error{where}, so Python never starts running it:\n\n"
                    f"```\n{exc}\n```"
                )
            return True
        result = run_python(snippet)
        ok = result.exit_code == 0 and not result.timed_out
        detail = "timed_out" if result.timed_out else f"exit {result.exit_code}"
        t.tools.append(ToolCall("code_execution", ok, detail))
        t.reply = f"I ran that snippet locally.\n\n{_format_execution(result)}"
        return True
