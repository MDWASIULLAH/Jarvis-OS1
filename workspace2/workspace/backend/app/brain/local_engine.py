...
_CODE_REQUEST_RE = re.compile(
    r"\b(write|create|build|generate|make)\b.{0,40}\b(code|script|program|function|api|app|bot)\b|\btask\.code\b",
    re.IGNORECASE,
)
...
class LocalReasoningBackend(LLMBackend):
    ...
    def answer(self, prompt: str) -> str:
        ...
        if _CODE_REQUEST_RE.search(prompt):
            return self._code_request_reply(prompt)
    ...
    @staticmethod
    def _code_request_reply(question: str) -> str:
        cleaned = question.strip().rstrip(".!?")
        if re.search(r"\bpython\b", cleaned, re.IGNORECASE):
            return (
                "I can write this Python code now.\n\n"
                "Send these details so I generate exactly what you want:\n"
                "1) feature/goal,\n"
                "2) input and output format,\n"
                "3) file path where it should be added.\n\n"
                f"Current request: `{cleaned}`"
            )
        return (
            "I can write code for you right now.\n\n"
            "Tell me these 3 details and I will generate complete code:\n"
            "1) language/framework,\n"
            "2) expected behavior,\n"
            "3) input/output (or API contract).\n\n"
            "Example: `Node.js Express route POST /sum that accepts {a,b} and returns {result}`."
        )
