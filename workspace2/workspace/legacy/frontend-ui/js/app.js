...
const JARVIS_APP = (() => {
  ...
  function boot() {
    ...
    try {
      ...
      // 13. Workspace — coding editor
      JARVIS_WORKSPACE.init();
      console.log('[J.A.R.V.I.S.] Workspace online');
  ...
  function bindUIEvents() {
    ...
    function toggleFullscreen(enable) {
      ...
      // Keep hamburger menu accessible in fullscreen: clone it inside the console
      const fullscreenMenu = document.getElementById('fullscreenMenuBtn');
      if (isFs) {
        if (!fullscreenMenu && topMenuBtn) {
          const clone = topMenuBtn.cloneNode(true);
          clone.id = 'fullscreenMenuBtn';
          clone.title = 'Chat History';
          clone.setAttribute('aria-label', 'Chat History');
          clone.addEventListener('click', () => JARVIS_HISTORY.toggleSidebar());
          document.body.appendChild(clone);
        }
        const fm = document.getElementById('fullscreenMenuBtn');
        if (fm) fm.style.display = 'block';
        if (exitFsBtn) exitFsBtn.style.display = 'grid';
        if (fullscreenBtn) fullscreenBtn.style.display = 'none';
        JARVIS_UTILS.showToast('Fullscreen mode -- press Escape or ✕ to exit', 'info');
      } else {
        if (fullscreenMenu) fullscreenMenu.style.display = 'none';
        if (exitFsBtn) exitFsBtn.style.display = 'none';
        if (fullscreenBtn) fullscreenBtn.style.display = 'grid';
      }
    }
    if (fullscreenBtn) {
    ...
    if (wsBtn) {
      wsBtn.addEventListener('click', () => JARVIS_WORKSPACE.toggle());
    }
    ...
    if (speakToggleBtn) {
      function updateSpeakButton() {
        const enabled = JARVIS_UTILS.storageGet('speakEnabled', true);
        speakToggleBtn.textContent = enabled ? '🔊' : '🔇';
        speakToggleBtn.title = enabled ? 'Click to mute JARVIS' : 'Click to unmute JARVIS';
        speakToggleBtn.style.opacity = enabled ? '1' : '0.4';
      }
      updateSpeakButton();
      speakToggleBtn.addEventListener('click', () => {
        const currently = JARVIS_UTILS.storageGet('speakEnabled', true);
        JARVIS_UTILS.storageSet('speakEnabled', !currently);
        if (currently) JARVIS_VOICE.stop();
        updateSpeakButton();
        JARVIS_UTILS.showToast(currently ? 'JARVIS is now silent' : 'JARVIS can speak now', 'info');
      });
    }
...
if (topMenuBtn) {
  topMenuBtn.addEventListener('click', () => {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Ctrl/Cmd + W for workspace
      if ((e.ctrlKey || e.metaKey) && e.key === 'w') {
        e.preventDefault();
        JARVIS_WORKSPACE.toggle();
      }
